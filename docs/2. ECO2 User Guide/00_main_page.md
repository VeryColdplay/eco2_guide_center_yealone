...
난방, 냉방, 급탕, 조명, 환기에 대한 에너지소요량을 전반적으로 계산하고 이를 바닥면적으로 나눈 값을 사용하여 에너지 성능에 따라 10개 등급(1+++ ~ 7)으로 평가합니다. 평가를 위한 분석 툴은 **ECO2 프로그램**을 사용합니다. 
...
<div div style="
  ...
  margin-top: 20px;
">
        <strong>건축물 에너지효율등급 인증기준</strong><br>
        $$
...
<strong> To. 한희
위의 수식이 렌더링이 안되는 문제가 있어서 다이어그램으로 표현해야할 것 같아요. 단위면적당 1차 에너지 소요량이 5개 계산을 모두 합친 결과라는 것을 보여줄 수 있는 다이어그램 그려주세요! </strong>
